...
class WindowManager implements IWindowManager {
    ...
    public showWindow(windowType: WindowType): void {
        ...
        // 恢复显示时校正位置，防止缩到屏幕边缘
        if (windowType === 'minimode') {
            const pos = { ...win.getBounds() };
            if (normalizeWindowPosition(win, pos)) {
    ...
    private createMiniModeWindow(): void {
        // 无坐标创建（传 x/y 在某些平台上不可靠，ready-to-show 后 setPosition）
        const primary = screen.getPrimaryDisplay();
        const { x: wX, y: wY, width: wW, height: wH } = primary.workArea;
        const centerX = Math.round(wX + (wW - MINIMODE_WIDTH) / 2);
        const centerY = Math.round(wY + (wH - MINIMODE_HEIGHT) / 2);
        ...
        miniWindow.once('ready-to-show', () => {
            ...
            if (
                ...
                const isVisible = displays.some((d) => {
                    ...
                    return (
                        savedPosition.x >= dx - MINIMODE_WIDTH + 20 &&
                        savedPosition.x <= dx + dw - 20 &&
                        savedPosition.y >= dy - MINIMODE_HEIGHT + 20 &&
                        savedPosition.y <= dy + dh - 20
                    );
                });
                if (isVisible) {
                    miniWindow.setBounds({ x: savedPosition.x, y: savedPosition.y });
                    return;
                }
            }
            // 居中
            miniWindow.setPosition(centerX, centerY);
        });
        ...
        // 关闭迷你模式时同步设置，避免下次启动又弹出来
        miniWindow.on('close', () => {
            appConfig.setConfig({ 'private.minimode': false });
        });
