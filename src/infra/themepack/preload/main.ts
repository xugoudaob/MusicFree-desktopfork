...
/**
 * 计算 RGB 颜色的相对亮度（WCAG relative luminance）
 * 返回 0（纯黑）到 1（纯白）之间的值。
...
/**
 * 生成主题变量兜底 CSS。
 * 根据 --color-bg-base 的亮度自动选择浅色系或深色系文字色，
 * 确保任何缺变量的主题包都有可读的文字颜色。
 */
function generateFallbackCss(): string {
    const luminance = getBgLuminance();
    ...
    :root {
        ...
        --color-text-brand: rgb(4, 120, 87);
        --color-fill-brand: rgb(5, 150, 105);
        --color-fill-brand-hover: rgba(5, 150, 105, 0.85);
        --color-border-default: rgba(0, 0, 0, 0.13);
        --color-border-subtle: rgba(0, 0, 0, 0.08);
        --color-favorite: rgb(239, 68, 68);
        --color-status-success-text: rgb(21, 128, 61);
        --color-status-info-text: rgb(2, 132, 199);
        --color-status-warn-text: rgb(180, 83, 9);
        --color-status-danger-text: rgb(220, 38, 38);
    }
}`;
    }
    ...
    :root {
        ...
        --color-text-secondary: rgb(160, 160, 170);
        --color-text-muted: rgba(255, 255, 255, 0.55);
        --color-text-placeholder: rgba(255, 255, 255, 0.5);
        --color-text-disabled: rgba(255, 255, 255, 0.22);
        --color-text-on-brand: rgb(18, 18, 18);
        --color-text-brand: rgb(167, 139, 250);
        --color-fill-brand: rgb(167, 139, 250);
        --color-fill-brand-hover: rgba(167, 139, 250, 0.85);
        --color-border-default: rgba(255, 255, 255, 0.12);
        --color-border-subtle: rgba(255, 255, 255, 0.07);
        --color-favorite: rgb(239, 68, 68);
        --color-status-success-text: rgb(110, 231, 183);
        --color-status-info-text: rgb(34, 211, 238);
        --color-status-warn-text: rgb(251, 191, 36);
        --color-status-danger-text: rgba(239, 68, 68, 0.8);
    }
}`;
}

/** 注入或更新 <style> 节点（含兜底 CSS） */
function injectCss(css: string): void {
    let cssWithFallback = css;
    try {
        cssWithFallback = css + generateFallbackCss();
    } catch {
        // generateFallbackCss 出错时仅注入主题 CSS（global.scss 的 @layer fallback 会兜底）
    }
    ...
    styleNode.textContent = cssWithFallback;
}
